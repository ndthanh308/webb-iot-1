...
// import { ChartContainer, ChartTooltip, ChartTooltipContent } from "@/components/ui/chart"
// import { iotDataService, IoTSensorData } from "@/lib/iot-service"
...
// // Utility function to safely convert and validate chart data
// const prepareChartData = (historicalData: IoTHistoryEntry[]) => {
//   console.log('🔍 DEBUG: prepareChartData input:', {
//     length: historicalData.length,
//     firstEntry: historicalData[0],
//     lastEntry: historicalData[historicalData.length - 1]
//   })
  
//   // Always prioritize Firebase data if available
//   if (historicalData.length > 0) {
//     // Process real historical data from Firebase
//     const processedData = historicalData.slice(-24).map((entry, index) => {
//       // Safe number conversion with more aggressive fallbacks
//       const humidity = (entry.humidity !== undefined && entry.humidity !== null) ? Number(entry.humidity) : 45;
//       const temperature = (entry.temperature !== undefined && entry.temperature !== null) ? Number(entry.temperature) : 25;
//       const food = (entry.food_percentage !== undefined && entry.food_percentage !== null) ? Number(entry.food_percentage) : 50;
//       const water = (entry.water_percentage !== undefined && entry.water_percentage !== null) ? Number(entry.water_percentage) : 60;
      
//       // Validate and clamp values
//       const validHumidity = !isNaN(humidity) ? Math.max(0, Math.min(100, Number(humidity.toFixed(1)))) : 45;
//       const validTemperature = !isNaN(temperature) ? Math.max(-10, Math.min(50, Number(temperature.toFixed(1)))) : 25;
//       const validFood = !isNaN(food) ? Math.max(0, Math.min(100, Math.round(food))) : 50;
//       const validWater = !isNaN(water) ? Math.max(0, Math.min(100, Math.round(water))) : 60;
      
//       // Debug log for raw vs processed data
//       if (index < 3) { // Only log first 3 entries to avoid spam
//         console.log(`🔍 DEBUG Entry ${index}:`, {
//           raw: { humidity: entry.humidity, temperature: entry.temperature, food: entry.food_percentage, water: entry.water_percentage },
//           processed: { validHumidity, validTemperature, validFood, validWater }
//         })
//       }
...
//   // Return sample data when no historical data available
//   return Array.from({ length: 24 }, (_, i) => {
//     const hour = i;
//     const baseTemp = 25;
//     const baseHumidity = 45;
//     const tempVariation = Math.sin((hour / 24) * 2 * Math.PI) * 5;
//     const humidityVariation = Math.sin(((hour + 12) / 24) * 2 * Math.PI) * 10;
    
//     return {
//       time: `${hour.toString().padStart(2, '0')}:00`,
//       humidity: Number((baseHumidity + humidityVariation + (Math.random() - 0.5) * 4).toFixed(1)),
//       temperature: Number((baseTemp + tempVariation + (Math.random() - 0.5) * 2).toFixed(1)),
//       food: Math.max(0, Math.round(80 - (hour * 2) + (Math.random() * 10))),
//       water: Math.max(0, Math.round(90 - (hour * 1.5) + (Math.random() * 8)))
//     }
//   })
// }
...
//   // Destructure sensor data for easier access with safe fallbacks
//   const { 
//     food_percentage: rawFoodPercentage = 0, 
//     water_percentage: rawWaterPercentage = 0, 
//     temperature: rawTemperature = 0, 
//     humidity: rawHumidity = 0, 
//     air_quality_voltage: rawAirQualityVoltage = 0 
//   } = sensorData
  
//   // Safely convert to numbers and handle NaN
//   const foodPercentage = isNaN(Number(rawFoodPercentage)) ? 0 : Number(rawFoodPercentage);
//   const waterPercentage = isNaN(Number(rawWaterPercentage)) ? 0 : Number(rawWaterPercentage);
//   const temperature = isNaN(Number(rawTemperature)) ? 0 : Number(rawTemperature);
//   const humidity = isNaN(Number(rawHumidity)) ? 0 : Number(rawHumidity);
//   const airQualityVoltage = isNaN(Number(rawAirQualityVoltage)) ? 0 : Number(rawAirQualityVoltage);
...
//         console.log('🔍 First 3 historical entries:', history.slice(0, 3))
...
//           // Debug: Log first few entries to check data structure
//           if (data.length > 0) {
//             console.log('🔍 Sample data entries:', data.slice(0, 3).map(entry => ({
//               timestamp: new Date(entry.timestamp).toLocaleString(),
//               temperature: entry.temperature,
//               humidity: entry.humidity,
//               food_percentage: entry.food_percentage,
...
//                       temperature < 15 ? 'text-blue-400 bg-blue-400/20' :
//                       temperature <= 22 ? 'text-cyan-400 bg-cyan-400/20' :
//                       temperature <= 27 ? 'text-green-400 bg-green-400/20' :
//                       'text-red-400 bg-red-400/20'
//                     }`}>
//                       {temperature < 15 ? 'Lạnh' : temperature <= 22 ? 'Mát' : temperature <= 27 ? 'Bình thường' : 'Nóng'}
//                     </div>
//                   </div>
...
//                     <div className={`text-xs px-2 py-1 rounded-full mt-1 ${
//                       foodPercentage === 0 ? 'text-red-400 bg-red-400/20' :
...
//                     }`}>
//                       {foodPercentage === 0 ? 'Hết' : foodPercentage < 20 ? 'Sắp hết' : foodPercentage < 50 ? 'Cần thêm' : 'Đủ'}
//                     </div>
//                   </div>
...
//                     <div className={`text-xs px-2 py-1 rounded-full mt-1 ${
//                       waterPercentage === 0 ? 'text-red-400 bg-red-400/20' :
...
//                     }`}>
//                       {waterPercentage === 0 ? 'Hết' : waterPercentage < 20 ? 'Sắp hết' : waterPercentage < 50 ? 'Cần thêm' : 'Đủ'}
//                     </div>
//                   </div>
//                 </div>
//               </CardContent>
...
//                   Lần cuối: {formatLastFeedTime(lastFeedTime)}
//                 </div>
//                 {foodPercentage < 20 && (
...
//                 />
//                 <div className="text-xs text-gray-400 mt-1">
//                   Trạng thái: {waterPercentage > 80 ? 'Đầy' : waterPercentage > 50 ? 'Đủ dùng' : 'Cần đổ thêm'}
//                 </div>
//                 {waterPercentage < 20 && (
//                   <div className="text-xs text-red-400 mt-1 flex items-center">
//                     ⚠️ Nước sắp hết!
...
//                     </span>
//                   ) : (
//                     <span className="text-orange-400 bg-orange-400/20 px-2 py-1 rounded-full">
//                       Chờ dữ liệu
//                     </span>
//                   )}
//                 </div>
...
//             <CardContent>
//               <div className="mb-4">
//                 <h4 className="text-gray-300 mb-2">Biểu đồ thời gian (24h gần nhất)</h4>
//                 <ChartContainer
//                   config={{
//                     humidity: {
//                       label: "Độ ẩm",
...
//                     <span className="flex items-center gap-1">
//                       <span className="w-3 h-3 rounded-full bg-blue-500"></span> Độ ẩm (%)
//                     </span>
//                     <span className="flex items-center gap-1">
//                       <span className="w-3 h-3 rounded-full bg-orange-500"></span> Nhiệt độ (°C)
//                     </span>
//                   </div>
...
//               {/* Biểu đồ thức ăn và nước */}
//               <div className="mb-4">
//                 <h4 className="text-gray-300 mb-2">Mức thức ăn & nước (24h gần nhất)</h4>
//                 <ChartContainer
//                   config={{
//                     food: {
//                       label: "Thức ăn",
...
//                   <div className="flex gap-4">
//                     <span className="flex items-center gap-1">
//                       <span className="w-3 h-3 rounded-full bg-purple-500"></span> Thức ăn (%)
//                     </span>
//                     <span className="flex items-center gap-1">
//                       <span className="w-3 h-3 rounded-full bg-cyan-500"></span> Nước (%)
//                     </span>
//                   </div>
//                   <div className="text-gray-500">
//                     📊 Hiện thị xu hướng sử dụng
//                   </div>
//                 </div>
//               </div>
...
// dashboard.tsx - Updated for refactored lib services
"use client"
...
import { iotDataService, IoTControlData } from '@/lib/iot-service'
import { IoTSensorData, mqttService } from '@/lib/mqtt-service'
import { controlCommands } from '@/lib/mqtt-commands'
...
export default function Dashboard({ user, onLogout }: DashboardProps) {
  ...
  const [controlData, setControlData] = useState<IoTControlData>({
    auto_feeding: false,
    area_sensor: true,
    laser_game: false,
    ac_temperature: 25
  ...
  useEffect(() => {
    ...
    const initializeSystem = async () => {
      try {
        console.log('🚀 Initializing Dashboard with refactored services...')
        
        // 1. Initialize IoT system
        await iotDataService.initializeRealDataMode()
        
        // 2. Subscribe to sensor data from ESP32 via MQTT + Firebase
        unsubscribeSensor = iotDataService.subscribeSensorData((data: IoTSensorData) => {
          console.log('📊 Dashboard sensor data updated:', {
            temp: `${data.temperature.toFixed(1)}°C`,
            humidity: `${data.humidity.toFixed(1)}%`,
            food: `${data.food_percentage}%`,
            water: `${data.water_percentage}%`,
            air: `${data.air_quality_voltage.toFixed(2)}V`
          })
          setSensorData(data)
        })
        
        // 3. Subscribe to control data from Firebase
        unsubscribeControl = iotDataService.subscribeControlData((data: IoTControlData) => {
          console.log('🎮 Dashboard control data updated:', data)
          setControlData(data)
        })
        
        // 4. Subscribe to chart data from Firebase history
        setIsChartLoading(true)
        unsubscribeChart = firebaseDataService.subscribeToChartData((data: ChartDataPoint[]) => {
          console.log('📈 Dashboard chart data updated:', data.length, 'points')
          setChartData(data)
          setIsChartLoading(false)
        })
        
        // 5. Subscribe to MQTT connection status
        unsubscribeConnection = mqttService.subscribeConnectionStatus((connected: boolean) => {
  ...
  const handleFeeding = useCallback(async () => {
    setIsLoading(true)
    try {
      await controlCommands.feeding()
      console.log('✅ Feeding command sent successfully')
    } catch (error) {
      console.error('❌ Feeding command failed:', error)
    } finally {
      setTimeout(() => setIsLoading(false), 1000) // UI feedback
    }
  }, [])

  const handleIncreaseTemp = useCallback(async () => {
    setIsLoading(true)
    try {
      await controlCommands.increaseTemp()
      console.log('✅ Temperature increased')
    } catch (error) {
      console.error('❌ Temperature increase failed:', error)
    } finally {
      setTimeout(() => setIsLoading(false), 500)
    }
  }, [])

  const handleDecreaseTemp = useCallback(async () => {
    setIsLoading(true)
    try {
      await controlCommands.decreaseTemp()
      console.log('✅ Temperature decreased')
    } catch (error) {
      console.error('❌ Temperature decrease failed:', error)
    } finally {
      setTimeout(() => setIsLoading(false), 500)
    }
  }, [])
  ...
  const handleLaserToggle = useCallback(async () => {
    try {
      const newState = !controlData.laser_game
      await controlCommands.toggleLaser(newState)
      console.log(`✅ Laser game ${newState ? 'enabled' : 'disabled'}`)
    } catch (error) {
      console.error('❌ Laser toggle failed:', error)
    }
  }, [controlData.laser_game])

  const handleAreaSensorToggle = useCallback(async () => {
    try {
      const newState = !controlData.area_sensor
      await controlCommands.toggleAreaSensor(newState)
      console.log(`✅ Area sensor ${newState ? 'enabled' : 'disabled'}`)
    } catch (error) {
      console.error('❌ Area sensor toggle failed:', error)
    }
  }, [controlData.area_sensor])
  ...
  const prepareChartData = useCallback(() => {
    if (!chartData || chartData.length === 0) {
      ...
      return Array.from({ length: 24 }, (_, i) => ({
        ...
        temperature: 24 + Math.sin(i / 4) * 3 + (Math.random() - 0.5) * 2,
        humidity: 45 + Math.cos(i / 3) * 10 + (Math.random() - 0.5) * 5,
        food: Math.max(0, 80 - i * 2 + (Math.random() - 0.5) * 10),
        water: Math.max(0, 90 - i * 1.5 + (Math.random() - 0.5) * 8)
      }))
    }
  ...
  const airQualityInfo = getAirQualityStatus(air_quality_voltage)
  const chartDataForRender = prepareChartData()
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="ml-16 p-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                {isLoading && (
                  <span className="text-sm text-blue-400 bg-blue-400/20 px-2 py-1 rounded-full animate-pulse">
                    Processing...
                  </span>
                )}
              </div>
            ...
            <p className="text-gray-400">
              Cập nhật hệ thống chăm sóc lần cuối: {new Date(last_updated).toLocaleString('vi-VN')} 
              ({formatLastUpdateTime(last_updated)})
            </p>
          </div>
        </div>
            ...
            <Card className="bg-gray-800 border-gray-700 h-full">
              <CardHeader>
                <CardTitle className="text-white flex items-center">
                  ...
                  Giám sát môi trường và Thiết bị
                  {cloudConnected && (
                    <span className="ml-2 text-xs text-green-400 bg-green-400/20 px-2 py-1 rounded-full">
                      Real-time
                    </span>
                  )}
                </CardTitle>
              ...
              <CardContent className="h-full flex flex-col">
                <div className="grid grid-cols-3 gap-6 flex-1 items-center">
                    ...
                    <div className="relative w-24 h-24 mx-auto mb-2">
                      <svg className="w-24 h-24 transform -rotate-90" viewBox="0 0 100 100">
                        ...
                        <circle
                          ...
                          strokeWidth="8"
                          fill="none"
                          strokeDasharray={`${Math.min(100, Math.max(0, temperature * 2)) / 100 * 251.2} 251.2`}
                          strokeLinecap="round"
                        />
                      </svg>
                      <div className="absolute inset-0 flex items-center justify-center">
                  ...
                  <div className="text-center flex flex-col justify-center h-full">
                    <div className="relative w-24 h-24 mx-auto mb-2">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="text-center">
                          <div className="text-2xl font-bold text-white">{humidity.toFixed(1)}</div>
                          <div className="text-xs text-gray-400">%</div>
                        </div>
                      </div>
                    </div>
                    <div className="text-sm text-gray-300">Độ ẩm phòng</div>
                    <div className={`text-xs px-2 py-1 rounded-full mt-1 ${
                      humidity < 30 ? 'text-orange-400 bg-orange-400/20' :
                      humidity > 70 ? 'text-blue-400 bg-blue-400/20' :
                  ...
                  <div className="text-center flex flex-col justify-center h-full">
                    <div className="relative w-24 h-24 mx-auto mb-2">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="text-center">
                          <div className="text-2xl font-bold text-white">{air_quality_voltage.toFixed(2)}</div>
                ...
                <div className="grid grid-cols-3 gap-6 flex-1 items-center">
                  ...
                  <div className="text-center flex flex-col justify-center h-full">
                    <div className="relative w-24 h-24 mx-auto mb-2">
                      <svg className="w-24 h-24 transform -rotate-90" viewBox="0 0 100 100">
                        ...
                        <circle
                          ...
                          cy="50"
                          r="40"
                          stroke={food_percentage < 20 ? "rgb(239, 68, 68)" : "rgb(59, 130, 246)"}
                          strokeWidth="8"
                          fill="none"
                          strokeDasharray={`${(food_percentage / 100) * 251.2} 251.2`}
                          strokeLinecap="round"
                          className={food_percentage < 20 ? "animate-pulse" : ""}
                        />
                      </svg>
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="text-center">
                          <div className="text-2xl font-bold text-white">{Math.round(food_percentage)}</div>
                          <div className="text-xs text-gray-400">%</div>
                        </div>
                      </div>
                    </div>
                    <div className="text-sm text-gray-300">Mức thức ăn</div>
                    <div className={`text-xs px-2 py-1 rounded-full mt-1 ${
                      food_percentage === 0 ? 'text-red-400 bg-red-400/20' :
                      food_percentage < 20 ? 'text-red-400 bg-red-400/20' : 
                      food_percentage < 50 ? 'text-yellow-400 bg-yellow-400/20' : 'text-green-400 bg-green-400/20'
                    }`}>
                      {food_percentage === 0 ? 'Hết' : food_percentage < 20 ? 'Sắp hết' : food_percentage < 50 ? 'Cần thêm' : 'Đủ'}
                    </div>
                  </div>
                  ...
                  <div className="text-center flex flex-col justify-center h-full">
                    <div className="relative w-24 h-24 mx-auto mb-2">
                      <svg className="w-24 h-24 transform -rotate-90" viewBox="0 0 100 100">
                        ...
                        <circle
                          ...
                          cy="50"
                          r="40"
                          stroke={water_percentage < 20 ? "rgb(239, 68, 68)" : "rgb(6, 182, 212)"}
                          strokeWidth="8"
                          fill="none"
                          strokeDasharray={`${(water_percentage / 100) * 251.2} 251.2`}
                          strokeLinecap="round"
                          className={water_percentage < 20 ? "animate-pulse" : ""}
                        />
                      </svg>
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="text-center">
                          <div className="text-2xl font-bold text-white">{Math.round(water_percentage)}</div>
                          <div className="text-xs text-gray-400">%</div>
                        </div>
                      </div>
                    </div>
                    <div className="text-sm text-gray-300">Mức nước</div>
                    <div className={`text-xs px-2 py-1 rounded-full mt-1 ${
                      water_percentage === 0 ? 'text-red-400 bg-red-400/20' :
                      water_percentage < 20 ? 'text-red-400 bg-red-400/20' : 
                      water_percentage < 50 ? 'text-yellow-400 bg-yellow-400/20' : 'text-green-400 bg-green-400/20'
                    }`}>
                      {water_percentage === 0 ? 'Hết' : water_percentage < 20 ? 'Sắp hết' : water_percentage < 50 ? 'Cần thêm' : 'Đủ'}
                    </div>
                  </div>
                    ...
                    <div className="relative w-24 h-24 mx-auto mb-2">
                      <svg className="w-24 h-24 transform -rotate-90" viewBox="0 0 100 100">
                        ...
                        <circle
                          ...
                          strokeWidth="8"
                          fill="none"
                          strokeDasharray={`${((controlData.ac_temperature - 16) / (30 - 16)) * 251.2} 251.2`}
                          strokeLinecap="round"
                        />
                      </svg>
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="text-center">
                          <div className="text-2xl font-bold text-white">{controlData.ac_temperature}</div>
                          <div className="text-xs text-gray-400">°C</div>
                        </div>
                      </div>
                    </div>
                    <div className="text-sm text-gray-300">Điều hòa</div>
                    <div className="flex gap-1 mt-2">
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={handleDecreaseTemp}
                        disabled={isLoading || controlData.ac_temperature <= 16}
                      ...
                      <Button
                        ...
                        disabled={isLoading || controlData.ac_temperature >= 30}
                        className="h-6 w-6 p-0 border-gray-600 hover:bg-gray-700"
                      >
            ...
            <Card className="bg-gray-800 border-gray-700">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-gray-300">Cho ăn tự động</span>
                  <button
                    onClick={() => controlCommands.toggleLaser(controlData.auto_feeding)}
                    className={`w-12 h-6 rounded-full relative transition-all duration-300 ease-in-out transform hover:scale-105 active:scale-95 ${
                      controlData.auto_feeding ? "bg-blue-600 hover:bg-blue-500" : "bg-gray-600 hover:bg-gray-500"
                    }`}
                  >
                    <div
                      className={`w-5 h-5 bg-white rounded-full absolute top-0.5 transition-all duration-300 ease-in-out shadow-lg ${
                        controlData.auto_feeding ? "right-0.5 transform" : "left-0.5"
                      }`}
                    />
                  </button>
                </div>
                ...
                {/* Area Sensor Toggle */}
                <div className="flex items-center justify-between">
                  <span className="text-gray-300">Cảm biến khu vực</span>
                  <button
                    onClick={handleAreaSensorToggle}
                    className={`w-12 h-6 rounded-full relative transition-all duration-300 ease-in-out transform hover:scale-105 active:scale-95 ${
                      controlData.area_sensor ? "bg-blue-600 hover:bg-blue-500" : "bg-gray-600 hover:bg-gray-500"
                    }`}
                  >
                    <div
                      className={`w-5 h-5 bg-white rounded-full absolute top-0.5 transition-all duration-300 ease-in-out shadow-lg ${
                        controlData.area_sensor ? "right-0.5" : "left-0.5"
                      }`}
                    />
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className={`w-12 h-6 rounded-full relative transition-all duration-300 ease-in-out transform hover:scale-105 active:scale-95 ${
                      controlData.laser_game ? "bg-blue-600 hover:bg-blue-500" : "bg-gray-600 hover:bg-gray-500"
                    }`}
                  >
                    <div
                      className={`w-5 h-5 bg-white rounded-full absolute top-0.5 transition-all duration-300 ease-in-out shadow-lg ${
                        controlData.laser_game ? "right-0.5" : "left-0.5"
                      }`}
                    />
                  </button>
                </div>
                ...
                <div className="mt-6">
                  <Button
                    ...
                    disabled={isLoading}
                    className="w-full bg-purple-600 hover:bg-purple-500 active:bg-purple-700 transition-all duration-200 transform hover:scale-[1.02] active:scale-[0.98] shadow-lg hover:shadow-xl"
                  >
                    {isLoading ? (
                ...
                <div className="mt-6 space-y-2 text-sm text-gray-400">
                  <div>
                    Trạng thái kết nối: <span className={cloudConnected ? "text-green-400" : "text-red-400"}>
                      {cloudConnected ? "Đã kết nối" : "Mất kết nối"}
                    </span>
                  </div>
                  <div>
                    ESP32 Status: <span className={cloudConnected ? "text-green-400" : "text-orange-400"}>
                      {cloudConnected ? "Online" : "Offline"}
                    </span>
                  </div>
                  <div>
                    Thiết bị hoạt động: <span className="text-white">
                      {Object.values(controlData).filter(Boolean).length}/4
                    </span>
                  </div>
                </div>
              </CardContent>
        ...
        {/* Bottom Row - Food/Water Status & Charts */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
          {/* Food and Water Status */}
          <Card className="bg-gray-800 border-gray-700">
            <CardHeader>
            ...
            <CardContent className="space-y-6">
              <div>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-gray-300">Thức ăn khô</span>
                  <span className={`${
                    food_percentage < 20 ? 'text-red-400 animate-pulse' : 
                    food_percentage < 50 ? 'text-yellow-400' : 'text-green-400'
                  }`}>
                    {Math.round(food_percentage)}%
                    {food_percentage < 20 && <span className="ml-1">⚠️</span>}
                  </span>
                </div>
                <Progress 
                  value={food_percentage} 
                  className={`h-2 bg-gray-700 ${food_percentage < 20 ? 'animate-pulse' : ''}`}
                />
                <div className="text-xs text-gray-400 mt-1">
                  Cập nhật: {formatLastUpdateTime(last_updated)}
                </div>
                {food_percentage < 20 && (
                  <div className="text-xs text-red-400 mt-1 flex items-center">
                    ⚠️ Thức ăn sắp hết! Cần đổ thêm.
                  </div>
                )}
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-gray-300">Nước uống</span>
                  <span className={`${
                    water_percentage < 20 ? 'text-red-400 animate-pulse' : 
                    water_percentage < 50 ? 'text-yellow-400' : 'text-green-400'
                  }`}>
                    {Math.round(water_percentage)}%
                    {water_percentage < 20 && <span className="ml-1">⚠️</span>}
                  </span>
                </div>
                <Progress 
                  value={water_percentage} 
                  className={`h-2 bg-gray-700 ${water_percentage < 20 ? 'animate-pulse' : ''}`}
                />
                <div className="text-xs text-gray-400 mt-1">
                  Trạng thái: {water_percentage > 80 ? 'Đầy' : water_percentage > 50 ? 'Đủ dùng' : 'Cần đổ thêm'}
                </div>
                {water_percentage < 20 && (
                  <div className="text-xs text-red-400 mt-1 flex items-center">
                    ⚠️ Nước sắp hết! Cần đổ thêm.
                  </div>
                )}
              </div>

              <div className="text-xs text-gray-400 space-y-1">
                <div>⏰ Lịch cho ăn tự động: {controlData.auto_feeding ? "Bật" : "Tắt"}</div>
                <div>🎮 Trò chơi laser: {controlData.laser_game ? "Đang chạy" : "Đã tắt"}</div>
                <div>
                  🚨 Cảnh báo sắp hết: <span className="text-orange-400">Dưới 20%</span>
                </div>
              </div>
          ...
          <Card className="bg-gray-800 border-gray-700">
            <CardHeader>
              <CardTitle className="text-white flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 text-xs">
                  {isChartLoading ? (
                    <span className="text-yellow-400 bg-yellow-400/20 px-2 py-1 rounded-full animate-pulse">
                      Đang tải...
                    </span>
                  ) : chartData.length > 0 ? (
                    <span className="text-green-400 bg-green-400/20 px-2 py-1 rounded-full">
            ...
            <CardContent>
              {isChartLoading ? (
                ...
                <>
                  ...
                  <div className="mb-6">
                    <h4 className="text-gray-300 mb-2 flex items-center gap-2">
                      🌡️ Biểu đồ nhiệt độ & độ ẩm
                      {chartData.length > 0 && (
                    ...
                    <ChartContainer
                      config={{
                        ...
                        temperature: { label: "Nhiệt độ (°C)", color: "#f97316" }
                      }}
                      className="h-[200px] w-full"
                    >
                      <ResponsiveContainer width="100%" height="100%">
                  ...
                  {/* Food & Water Chart */}
                  <div className="mb-4">
                    <h4 className="text-gray-300 mb-2">🍽️ Biểu đồ mức thức ăn & nước </h4>
                    <ChartContainer
                      config={{
                        food: { label: "Thức ăn (%)", color: "#8b5cf6" },
                        water: { label: "Nước (%)", color: "#06b6d4" }
                      }}
                      className="h-[160px] w-full"
                    >
                      <ResponsiveContainer width="100%" height="100%">
