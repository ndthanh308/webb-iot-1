...
// mqtt-service.ts - CLEAN VERSION - CHỈ LÀM MQTT
...
export const MQTT_TOPICS = {
  ...
  COMMANDS: {
    FEED: 'lalaland/food_and_drink/feed',
    WATER_REFILL: 'lalaland/water_refill',
    DECREASE_TEMP: 'lalaland/decreasetemp',
    INCREASE_TEMP: 'lalaland/increasetemp',
  },
  STATUS: {
    ESP32_ONLINE: 'lalaland/status/esp32_online',
    WEB_ONLINE: 'lalaland/status/web_online',
...
// 📡 MQTT SERVICE - CHỈ LÀM MQTT COMMUNICATION
class MQTTService {
  private client: any = null;
  private connected = false;
  private sensorData: Partial<IoTSensorData> = {};
  ...
  private tryNextBroker() {
    if (connectionAttempts >= MAX_CONNECTION_ATTEMPTS) {
      console.log(`❌ Max attempts reached. Retrying in 10 seconds...`);
      setTimeout(() => {
        connectionAttempts = 0;
        currentBrokerIndex = 0;
        this.connect();
  ...
  private handleMessage(topic: string, message: string) {
    try {
      // Handle sensor data and store to Firebase immediately
      switch (topic) {
        case MQTT_TOPICS.SENSORS.TEMPERATURE:
          const temperature = parseFloat(message);
          if (!isNaN(temperature)) {
            this.sensorData.temperature = temperature;
            this.storeSensorToFirebase('temperature', temperature);
        ...
        case MQTT_TOPICS.SENSORS.HUMIDITY:
          ...
          if (!isNaN(humidity)) {
            this.sensorData.humidity = humidity;
            this.storeSensorToFirebase('humidity', humidity);
        ...
        case MQTT_TOPICS.SENSORS.VOLTAGE:
          ...
          if (!isNaN(voltage)) {
            this.sensorData.air_quality_voltage = voltage;
            this.storeSensorToFirebase('air_quality_voltage', voltage);
            console.log(`🌬️ Air Quality: ${voltage}V`);
          }
          break;
        case MQTT_TOPICS.SENSORS.FOOD_STATUS:
          const foodPercentage = parseInt(message);
          if (!isNaN(foodPercentage)) {
            this.sensorData.food_percentage = foodPercentage;
            this.storeSensorToFirebase('food_percentage', foodPercentage);
            console.log(`🍽️ Food: ${foodPercentage}%`);
          }
          break;
        case MQTT_TOPICS.SENSORS.WATER_STATUS:
          const waterPercentage = parseInt(message);
          if (!isNaN(waterPercentage)) {
            this.sensorData.water_percentage = waterPercentage;
            this.storeSensorToFirebase('water_percentage', waterPercentage);
            console.log(`💧 Water: ${waterPercentage}%`);
          }
          break;
        case MQTT_TOPICS.STATUS.ESP32_ONLINE:
          const statusData = JSON.parse(message);
          ...
          break;
        default:
          console.log(`⚠️ Unknown topic: ${topic} = ${message}`);
          break;
      }
  ...
  private async storeSensorToFirebase(sensorType: string, value: number) {
    try {
      await firebaseDataService.storeIndividualSensor(sensorType, value);
      await firebaseDataService.storeSensorHistory(sensorType, value);
    } catch (error) {
      console.error(`❌ Failed to store ${sensorType} to Firebase:`, error);
    }
  }

  private async storeSensorDataToFirebase() {
    try {
      const dataToSave = {
        ...this.sensorData,
        last_updated: Date.now()
      ...
      console.error('❌ Failed to store current sensor data:', error);
    }
  }
